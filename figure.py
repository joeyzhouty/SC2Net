...
if __name__ == '__main__':
    ...
    steps = 167             # Number of steps fo GD between validation
    # steps = 30
    ...
    feed_val_lista = {"Z": zs_val, "Zp": z_curr, "X": sig_val, "label":lb,"lmbd": lmbd, "c_val": c_val-1e-10}
    ...
    #compute lmbds 
    # lmbds = [0.01,0.03,0.05,0.1,0.3,0.5,1]
    lmbds = [0.1]
    # lmbds = [] 
    ...
    for lmbd in lmbds:
        ...
        feed_val_lista = {"Z": zs_val, "Zp": z_curr, "X": sig_val, "label":lb.astype(np.float32),"lmbd": lmbd, "c_val": c_val-1e-10}
        # pb = MnistProblemGenerator(D, lmbd, batch_size=batch_size,
                                       # dir_mnist=save_dir, seed=42242)
        ...
        cost[4].append(lfis.cost(**feed_val_lista))
        acry[4].append(lfis.test_accuracy(pb,lmbd=lmbd))
        re_lfis = lfis.reconstruct(pb,lmbd=lmbd)
    import sys 
    sys.path.append("/data/home/dujw/darse/data_handlers")
    from reconstruct import reconstrcuct_drow
    reconstrcuct_drow(pb,lis = re_lis,lfis=re_lfis)
    # reconstrcuct_drow(pb,lis = re_lis,lfis=re_lfis,path="/data/home/dujw/darse/data_handlers/prediction_iter3_lmbd0.100000.pkl")

    # save_csv("cifar_lmbds_acry",columns,acry)
    # import pdb
    # pdb.set_trace()
    # save_csv("cifar_supervised_acry",columns,acry)


# computer layers
    layers = [4,7,12,21,35,59,100,128,145]
    # layers = []
    ...
    save_csv("mnist_layers_cost_18_10_17",columns,cost)
    import pdb
    pdb.set_trace()
