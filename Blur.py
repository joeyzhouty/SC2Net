...
if __name__ == '__main__':
    ...
    for lmbd in lmbds:
        ...
        # fista.optimize(X=sig_val, lmbd=lmbd, Z=zs_val,max_iter=100)
        # cost[2].append(fista.train_cost[-1])
        # acry[2].append(fista.test_accuracy(pb,lmbd=lmbd))


        lis.train(batch_provider=pb, max_iter=40, steps=steps, feed_val=feed_val_lista, reg_cost=8, tol=1,lr_init=lr_init)
    ...
    # save_csv("mnist_lmbds_acry",columns,acry)
    # save_csv("mnist_lmbds_cost",columns,cost)
